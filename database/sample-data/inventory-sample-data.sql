-- Sample/test fixtures for the Inventory module (Unit Measurements,
-- Warehouses, Products, Stock In, Stock Out). Stock In/Out/Transfer only
-- ever act on Inventory Items, so every seeded movement/balance here is
-- item_type INVENTORY_PRODUCT.
--
...
-- TIME of that movement (never user-entered) — see StockOutService.
INSERT INTO inventory_stock_movements (
  id, movement_number, movement_type, movement_subtype, item_type, item_id,
  warehouse_id, quantity, input_uom_id, unit_cost, total_cost, batch_number,
  expiry_date, reference_type, reference_id, notes, created_at
